...
{        
        include("../php/conn.php");
        $db_handle = pg_connect($strCnx_login);
...
function getInfoCapa($id_capa)
{
    include("../php/conn.php");
        ...
        if ($db_handle) 
        {
           ...
           $query2 = 'SELECT id_capa, nombre_capa, url, version_wms, nombre_fantasia, "singleTile", "isBaseLayer", "numZoomLevels", visibility, transparent, nivel_de_arbol, estilo_wms, version_wfs, select_control, estilo_wfs FROM atributos_por_capa where id_capa='.$id_capa.';'; 
           $result2 = pg_query($db_handle,$query2);
           if($result2){
               ...
               $row = pg_fetch_row($result2);
                ...
                if ($row){
                    ...
                    $bus = array(
                        ...
                        'estilo_wfs'=> $row[14], // cambiar por filter_wfs
                        'success' => true
                    );
                    array_push($json, $bus);
...
function guardaCapaNueva()
{
        ...
        if ($db_handle) 
        {
           ...
           { 
               if($row0[0] == 0)
               {
                    $query = "INSERT INTO atributos_por_capa(id_capa, nombre_capa, url,version_wms, nombre_fantasia, \"singleTile\",\"isBaseLayer\", \"numZoomLevels\",visibility, transparent, nivel_de_arbol,estilo_wms, version_wfs, select_control, estilo_wfs) VALUES (0,'".$info_capa['nombre_capa'] ."', '".$info_capa['url']."', '".$info_capa['version_wms']."', '".$info_capa['nombre_fantasia']."', '".$mosaico."', '".$capa_base."', '".$info_capa['zoom_level']."', '".$visibilidad."', '".$transparente."', '".$info_capa['nivel_arbol']."','".$info_capa['estilo_wms']."','".$info_capa['version_wfs']."', '".$seleccionable."', '".$info_capa['estilo_wfs']."');";          
...
function getConfInicialMapa($user_name){
        ...
        if ($db_handle) 
        {
           ...
           $query2 = "SELECT configura_mapa_x_usuario.id_capa_inicial,configura_mapa_x_usuario.extent_inicial FROM "+$db_schema_layers+"usuarios, "+$db_schema_layers+"configura_mapa_x_usuario WHERE configura_mapa_x_usuario.id_usuario = usuarios.id_usuario AND email LIKE '".$user_name."'"; 
           $result2 = pg_query($db_handle,$query2);
           if($result2){
                ...
                $bus = array(
                        'id_capa_inicial' => $row[0],
                        'extent_inicial' => array($row[1])
                );
                array_push($json, $bus);
